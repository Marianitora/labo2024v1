...
setwd("C:\\Users\\Mariano\\OneDrive\\Documentos\\Miscelaneas\\Austral\\2do cuatrimestre\\Lab de implementación") # Establezco el Working Directory
# cargo los datos
...
#agrego la clase binaria y elimino la clase ternaria
...
tb_grid_search <- data.table( vcp= numeric(),
                              vmin_bucket= numeric(),
                              max_depth = integer(),
                              min_split = integer(),
...
for (vmax_depth in c(6)) {
  for (vmin_split in c(400, 370, 340, 310 , 290, 250,220,210, 200))
    for (vmin_bucket in c(200, 185, 170, 155 , 145, 125,110,105, 100))
      for (vcp in c(-0.5)) {
